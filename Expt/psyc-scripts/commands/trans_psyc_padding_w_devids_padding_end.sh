...
seg_file(){
  output_folder=$RO_DATA_DIR/download/ori_splits/psyc_${1}_${3}_padding_end
  if [ ! -d $output_folder ]; then
    mkdir -p $output_folder
    echo "Seg full dialogue into windows with $1, dev_id_file=$4, $2 into $output_folder"
    python $ROOT_DIR/tensorflow/psyc_utils.py --padding_ahead --padding_end --cluster_strategy $1 --dev_id_file $4 --full_dial $2 --seg_dial_by_utterance $3 --output_folder $output_folder
  else
