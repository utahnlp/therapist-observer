...
if [ -e ${TRAINING_DIR} ]; then
  today=`date +%m-%d.%H:%M`
  mv ${TRAINING_DIR} ${TRAINING_DIR%?}_${today}
  echo "rename original training folder to "${TRAINING_DIR%?}_${today}
fi

mkdir -p ${TRAINING_DIR}
mkdir -p ${TRAINING_DIR}/models
mkdir -p ${TRAINING_DIR}/summary
mkdir -p ${TRAINING_DIR}/results
...
--model_prefix_to_restore=${MODEL_PREFIX_TO_RESTORE} \
...
--epochs=${EPOCH} \
--dropout_keep_prob=${DROP_KEEP_PROB} \
...
--result_dir=${TRAINING_DIR}/results/ \
...
"
python $ROOT_DIR/tensorflow/dial_run.py $pargs &> ${TRAINING_DIR}/train.log
