...
pay attention to that, remember to reprepare it
"""
...
class Vocab(object):
    ...
    def __init__(self, filename=None, initial_tokens=None, lower=False):
        ...
        self.unk_token = '<unk>'
        # Wilson: use this token to join question and title
        # Jie : it also used for join the speaker and the content of utterance.
        self.sep_token = '<sep>'
        ...
        self.initial_tokens.extend([self.pad_token, self.unk_token, self.sep_token, self.sos, self.eos])
    ...
    def load_pretrained_embeddings(self, embedding_path, entity_embedding_path=None):
        """
        ...
        if entity_embedding_path is not None:
            embedding_paths.append(entity_embedding_path)
        ...
        for token in filtered_tokens:
            self.add(token, cnt=0)
        # load embeddings, those padded tokenn has zero embedding.
        # if a token is not in the trained embeding, it is also OOV, training embedding
        # at this time will not distinguish the all the unknown tokens in the OOV.
        # @TODO: try random initial as the initial emebdding for OOV?
